...
if(FALSE){
  ...
  if(FALSE){
    ...
    res.NVI$Xclust.NVI_cov <- rdata$Xclust.NVI_cov
...
if(FALSE){
  ...
  ggsave(filename=path%+%title%+%"_[7]Age-LinePlot-Age=Total"%+%"-seed="%+%seed%+%",nclust="%+%nclust%+%".pdf", height=3, width=10)
  ...
  ggsave(filename=path%+%title%+%"_[7]Gender-LinePlot-Gender=Total"%+%"-seed="%+%seed%+%",nclust="%+%nclust%+%".pdf", height=3, width=10)
